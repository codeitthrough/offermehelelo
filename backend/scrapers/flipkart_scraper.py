...
class FlipkartScraper:
        ...
        try:
            ...
            for url in self.base_urls:
                try:
                    ...
                    # Find deal cards
                    deal_cards = soup.find_all(['div', 'a'], class_=re.compile(r'(product|Product|deal|Deal|card|Card)'))
    ...
    def parse_deal_card(self, card) -> Optional[Dict]:
        ...
        try:
            # Extract title
            title_elem = card.find(['a', 'div'], class_=re.compile(r'(title|Title|name|Name)'))
            if not title_elem:
            ...
            # Extract prices
            price_elem = card.find(['div'], class_=re.compile(r'(price|Price|cost|Cost)'))
            price_text = price_elem.get_text(strip=True) if price_elem else None
            discounted_price = self.extract_price(price_text) if price_text else None
            
            # Extract original price
            original_price_elem = card.find(['div'], class_=re.compile(r'(mrp|MRP|original|Original|strike)'))
            ...
            # If no original price, estimate
            if not original_price and discounted_price:
                original_price = discounted_price * 1.5
            
            # Extract discount percentage
            discount_elem = card.find(['div', 'span'], string=re.compile(r'\d+%\s*(off|Off|OFF)'))
            discount = 0
            if discount_elem:
